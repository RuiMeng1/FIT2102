...
// You'll need to:
//  - add a score property to the game State in state.ts DONE
...
function asteroids() {
  ...
  /** 
   * The merge of all the actions + ticks that this game expects into a single observable, making it more easier to manage
   */
  const action$: Observable<Action> = merge(tick$, startLeftRotate$,startRightRotate$,stopLeftRotate$,stopRightRotate$, startThrust$,stopThrust$,shoot$);
  /**
   * This takes the actions observables, pipes it into a scan
   * The scan function has the initial state of the accumulator stated as the initialState defined in state.ts
   * 
   * The intialState defines how the game starts off. The current state of the ship will therefore be changed by the keys
   */
  const state$: Observable<State> = action$.pipe(scan(reduceState, initialState));
  /**
   * Takes the states output and passes it through updateView, which changes the visual display
   * 
   * 
   */
