...
const deck$: Observable<Card>
 = range(0,suits.length).pipe(mergeMap(suitIndex => range(0, ranks.length).pipe(map(rankIndex => ({suit: suits[suitIndex], rank: ranks[rankIndex]})))));
 ...
 * 
 * @param a array
 * @param e element to be inserted into array
 */
function randomInsert<T>(a:ReadonlyArray<T>,e:T):ReadonlyArray<T> {
  const randomIndex = impureRandomNumberGenerator(a.length);
...
//   return range(0, deckCount).pipe(
//     mergeMap(() => deck$), // Emit each card in the deck
//     reduce((acc, card) => {
//       // Use randomInsert to shuffle cards
//       return randomInsert(acc, card);
//     }, [] as Card[]), // Initial value is an empty array
//     concatMap(deck => from(deck)) // Convert shuffled array back into a stream
//   ); 
// };
/**
